...
describe("registerWebTools — registration", () => {
	...
	it("web_search description names the currently configured active provider", () => {
		writeConfig({ provider: "tavily" });
		const { captured } = registerAndCapture();
		const description = captured.tools.get("web_search")?.description ?? "";
		// Root-cause guard: the active provider must reach the model so it omits
		// the provider arg instead of hard-coding brave (the enum's first literal).
		expect(description).toContain('active provider is "tavily"');
		expect(description).toContain("omit the provider argument");
	});

	it("web_search description falls back to brave when no provider is configured", () => {
		const { captured } = registerAndCapture();
		const description = captured.tools.get("web_search")?.description ?? "";
		expect(description).toContain('active provider is "brave"');
	});
});
